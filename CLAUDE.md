...
- **Migraciones:** `migrations/001` a `055`. Convención: BEGIN/COMMIT, idempotentes (IF NOT EXISTS / DROP IF EXISTS), retrocompatibles, query de verificación comentada al pie. **El SQL se corre en el SQL Editor ANTES de deployar el frontend.** Para dry-run contra prod: Management API con COMMIT→ROLLBACK.
