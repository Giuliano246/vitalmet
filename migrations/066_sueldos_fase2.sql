-- ═══════════════════════════════════════════════════════════════════
-- 066_sueldos_fase2.sql
-- Sueldos Fase 2: valor hora por empleado (parte de horas desde los
-- timers de Modo Planta — la atribución es por ORDEN: los timers usan
-- el usuario compartido de planta, el operario viene de
-- ordenes_produccion.operario ↔ empleados.operario_nombre) y
-- provisión mensual de SAC con consumo al liquidar el aguinaldo.
--
--   · empleados.valor_hora (ARS/hora, escala UOM) → el modal de
--     liquidación sugiere bruto = horas del período × valor_hora.
--   · Cuenta 214008 PROVISION SAC A PAGAR + config cta_provision_sac.
--   · confirmar_liquidacion RE-EMITIDA (base 065; ediciones futuras
--     parten de acá) con p_provisionar_sac:
--       - mensual/quincenas + tilde → asiento extra 'auto-prov-sac'
--         (debe sueldos por centro bruto/12, haber 214008). Se
--         provisiona SOLO el sueldo (las cargas del SAC van a gasto al
--         devengarlo — simplificación pyme deliberada).
--       - tipo 'sac' → el devengamiento consume la provisión: debe
--         214008 por min(saldo, gasto) y el resto a gasto por centro
--         (complemento exacto para que siempre balancee).
--     CAMBIA LA FIRMA (3 params) → DROP de la de 2 (patrón 059 con
--     guardar_permisos).
--   · anular_liquidacion RE-EMITIDA: también anula el asiento de
--     provisión ('liquidacion-prov-sac'). Los asientos de PAGO no se
--     tocan (plata que ya se movió — se corrigen a mano si corresponde).
-- Idempotente.
-- ═══════════════════════════════════════════════════════════════════
...
COMMENT ON COLUMN public.empleados.valor_hora IS
  'ARS por hora (escala UOM CCT 260/75 según categoría/rama). Usado para sugerir el bruto desde las horas de planta.';

-- ─── 2. Cuenta de provisión + config ─────────────────────────────────
INSERT INTO public.cuentas_contables (empresa_id, codigo, nombre, tipo, imputable, activa)
SELECT c.empresa_id, '214008', 'PROVISION SAC A PAGAR', 'pasivo', true, true
FROM public.config_contable c
WHERE NOT EXISTS (SELECT 1 FROM public.cuentas_contables x
                  WHERE x.empresa_id = c.empresa_id AND x.codigo = '214008');

ALTER TABLE public.config_contable
  ADD COLUMN IF NOT EXISTS cta_provision_sac uuid REFERENCES public.cuentas_contables(id) ON DELETE SET NULL;

UPDATE public.config_contable c SET cta_provision_sac = (
  SELECT id FROM public.cuentas_contables WHERE empresa_id=c.empresa_id AND codigo='214008')
  WHERE cta_provision_sac IS NULL;

-- ─── 3. confirmar_liquidacion (re-emisión, firma nueva) ──────────────
DROP FUNCTION IF EXISTS public.confirmar_liquidacion(uuid, numeric);
...
-- ─── 4. anular_liquidacion (re-emisión: anula también la provisión) ──
...
BEGIN
  ...
  IF l.estado = 'confirmada' THEN
    ...
    -- La provisión SAC de esta liquidación también se anula (el saldo
    -- de 214008 vuelve solo, porque el cálculo excluye anulados)
    UPDATE asientos SET estado = 'anulado'
...
-- ─── 5. Grants (la firma nueva de confirmar necesita los suyos) ──────
...
--     WHERE table_name='empleados' AND column_name='valor_hora') AS col_vh,
--   (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
--     WHERE n.nspname='public' AND p.proname='confirmar_liquidacion') AS rpc_confirmar,
--   (SELECT cta_provision_sac IS NOT NULL FROM config_contable LIMIT 1) AS cta_prov_ok;
-- Esperado: col_vh=1, rpc_confirmar=1 (la firma vieja fue DROPeada), cta_prov_ok=true
