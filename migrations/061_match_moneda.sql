-- ═══════════════════════════════════════════════════════════════════
-- MIGRACIÓN 061 — 3-way match: conversión de moneda + mensaje claro
--
-- Bug detectado en prod (OC-2026-009): el match comparaba el neto de
-- la factura contra el valor recibido EN LA MONEDA DE LA OC, sin
-- convertir. OC en USD + factura en ARS ⇒ discrepancia siempre.
-- Además, cuando la OC no tiene recepciones registradas el error era
-- un genérico "Match fallido: ... vs 0" que no decía qué hacer.
--
-- Cambios (re-emisión de registrar_factura_recibida, base = 054):
--   1. El valor recibido se convierte a la moneda de la factura con
--      TC = tipo_cambio de la factura, o el de la OC como fallback.
--      Monedas distintas sin TC ⇒ excepción clara (no bypasseable por
--      override: sin TC no hay comparación posible).
--   2. OC sin recepciones (Σ cantidad_recibida = 0) ⇒ mensaje que
--      indica registrar la recepción primero (override admin sigue
--      disponible para casos históricos).
--
-- Requiere: 054 (RPC v2, factura_recibida_impuestos). Idempotente.
...
-- ═══════════════════════════════════════════════════════════════════
-- 1) Una sola versión del RPC (4 args) y que menciona la conversión:
-- SELECT proname, pronargs FROM pg_proc WHERE proname='registrar_factura_recibida';
-- Esperado: 1 fila con pronargs = 4.
--
-- 2) La OC del bug — confirmar si tiene recepciones y en qué moneda está:
-- SELECT o.nro, o.moneda, o.tipo_cambio,
--        COALESCE(SUM(i.cantidad_recibida),0) AS qty_recibida,
--        COALESCE(SUM(i.precio_unitario*i.cantidad_recibida),0) AS valor_recibido
--   FROM public.ordenes_compra o
--   JOIN public.oc_items i ON i.oc_id = o.id
--  WHERE o.nro = 'OC-2026-009'
--  GROUP BY o.nro, o.moneda, o.tipo_cambio;
